...
if ($prijavaTip == administrator) {
    if (isset($_POST["sort"])) {
        ...
        $uzlsil = $_POST["uzlsil"];
        $sql = "select z.ID_zahtjeva, z.Broj_bodova, z.Status, z.Datum, k.Ime, k.Prezime, n.Naziv from "
                . "Zahtjev_pobjednik z, Korisnik k, Natjecanje n where "
                . "z.Korisnik_Korisnicko_ime=k.Korisnicko_ime and z.Natjecanje_ID_natjecanje=n.ID_natjecanje "
                . "and z.Status is not null order by $sortpo $uzlsil";
        $rs = $baza->upitDB($sql);
        if ($rs->num_rows > 0) {
            $poruka = "<a class='edit' href=\"zahtjevi_na_cekanju.php\">Zahtjevi na čekanju</a>"
                    . "<a class='edit' href=\"zahtjev_za_pobjednikom.php\">Novi zahtjev</a>";
            $poruka .= "<br><table id='tablica1' width='100%'><thead id='tablica_head'>"
                    . "<th>Broj zahtjeva</th><th>Ime i Prezime korisnika"
                    . "</th><th>Natjecanje</th><th>Broj bodova</th><th>Datum zahtjeva</th>"
                    . "<th>Status</th><th>Akcija</th></thead>";
            ...
            while ($red = $rs->fetch_assoc()) {
                ...
                $poruka .= '<tr><td>' . $red["ID_zahtjeva"] . '</td><td>' . $red["Ime"] . ' ' . $red["Prezime"] .
                        ...
                        $status . '</td>' . "<td><a href=\"edit_zahtjev.php?id=$red[ID_zahtjeva]\" onClick=\"return confirm('Sigurno želite urediti?')\">Uredi</a>"
                        . "<br><a href=\"delete_zahtjev.php?id=$red[ID_zahtjeva]\" onClick=\"return confirm('Sigurno želite obrisati?')\">Obriši</a></td>" . '</tr>';
            }
        ...
        $sql = "select z.ID_zahtjeva, z.Broj_bodova, z.Status, z.Datum, k.Ime, k.Prezime, n.Naziv from Zahtjev_pobjednik z, Korisnik k, Natjecanje n where z.Korisnik_Korisnicko_ime=k.Korisnicko_ime and z.Natjecanje_ID_natjecanje=n.ID_natjecanje and z.Status is not null";
        $rs = $baza->upitDB($sql);
        if ($rs->num_rows > 0) {
            $poruka = "<a class='edit' href=\"zahtjevi_na_cekanju.php\">Zahtjevi na čekanju</a>"
                    . "<a class='edit' href=\"zahtjev_za_pobjednikom.php\">Novi zahtjev</a>";
